...
//    println(factorial)
    ...
    println("Input a number between 2 and 12")
    val nBetween2And12 = readln().toInt()
    var combinations = 0
    var dice1 = Random.nextInt(6)
    while (dice1 <= 6) {
        var dice2 = Random.nextInt(6)
        while (dice2 <= 6) {
            if (dice1 + dice2 == nBetween2And12) {
                combinations += 1
            }
        }
    }
    var prob = combinations.toDouble() / 36 * 100
    println("The odds of rolling a $nBetween2And12 are $combinations in 36 of $prob%")
